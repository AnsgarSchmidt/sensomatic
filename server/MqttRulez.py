        ...
        if keys[0] == Room.BATH_ROOM:
            ...
            if keys[1] == "motion":
                ...
                if not self._redis.exists("PlayRadioInBath") and not self._redis.exists("shower") and not self._redis.exists("bath"):
                    ...
                    self._mqclient.publish("bathroom/ttsout", self._template.getCurrentTime())
                    s = Mpd().getServerbyName("Bath")
                    s.stop()
                    s.emptyPlaylist()
        ...
        if keys[0] == Room.ANSI_ROOM:
            ...
            if keys[1] == "motion":
                ...
                if self._redis.exists("ansiwakeup"):
                    ...
                    self._mqclient.publish("chromecast/Chromeansi/volume",       0.4)
                    self._mqclient.publish("chromecast/Chromeansi/playMusicURL", 'http://rb-bremenvier-live.cast.addradio.de/rb/bremenvier/live/mp3/128/stream.mp3')
            ...
            if keys[1] == "wan":
                if keys[2] == "rx":
                    if self._cortex_wan_rx > 0 and int(v) > self._cortex_wan_rx:
                        self._mqclient.publish("cortex/wan/rx-diff", int(int(v) - self._cortex_wan_rx))
                    self._cortex_wan_rx = int(v)
                if keys[2] == "tx":
                    if self._cortex_wan_tx > 0 and int(v) > self._cortex_wan_tx:
                        self._mqclient.publish("cortex/wan/tx-diff", int(int(v) - self._cortex_wan_tx))
                    self._cortex_wan_tx = int(v)

            if keys[1] == "cortex":
                if keys[2] == "rx":
                    if self._cortex_cortex_rx > 0 and int(v) > self._cortex_cortex_rx:
                        self._mqclient.publish("cortex/cortex/rx-diff", int(int(v) - self._cortex_cortex_rx))
            ...
            if keys[1] == "phawxansi":
                if keys[2] == "rx":
                    if self._cortex_phawxansi_rx > 0 and int(v) > self._cortex_phawxansi_rx:
                        self._mqclient.publish("cortex/phawxansi/rx-diff", int(int(v) - self._cortex_phawxansi_rx))
